...
FILE_BASE_DIRECTORY = '/Users/gautamve/workspace/s3_file/'
METRICS_FILENAME = 'top100Metrics000'
PlACEMENTS__FILENAME = 'top100Placementfeature000'
CONTENT_FILENAME = 'top100Contentfeature000'
RESOURCEBUNDLE_FILENAME = 'top100Bundlefeature000'
PLACEMENT_PROPERTIES_FILENAME = 'top100PlacementProperties000'
...
def loadDatasets(cleanDF):
	...
	metrics_df1 = pd.read_csv(FILE_BASE_DIRECTORY+PlACEMENTS__FILENAME, skiprows=0, header=None)
	...
	metrics_df2 = pd.read_csv(FILE_BASE_DIRECTORY+CONTENT_FILENAME, skiprows=0, header=None)
	...
	#metrics_df3 = pd.read_csv(FILE_BASE_DIRECTORY+RESOURCEBUNDLE_FILENAME, skiprows=0, header=None)
	#metrics_df3.columns = ['frozen_content_id', 'language_code']
	#if cleanDF:
	#	metrics_df3  = cleanDataframe(metrics_df3)

	metrics_df4 = pd.read_csv(FILE_BASE_DIRECTORY+PLACEMENT_PROPERTIES_FILENAME, skiprows=0, header=None)
	...
	return metrics_df1, metrics_df2, metrics_df4;
...
def saveModel(xg_reg, learning_rate_val, max_depth_val):
	filename =  DATA_BASE_DIRECTORY + 'xg_reg_model_02_01_2020_{}_{}.sav'
	filename  = filename.format(learning_rate_val, max_depth_val) 
	pickle.dump(xg_reg, open(filename, 'wb'))

def trainModel():
	DEGREE = 2
	df1, df2, df4 = loadDatasets(False)
	training_data_file = FILE_BASE_DIRECTORY + METRICS_FILENAME
	for chunk in pd.read_csv(training_data_file, chunksize=CHUNKSIZE):
		
		chunk.columns = ['frozen_placement_id', 'metrics_hour', 'impressions']

		#format the timestamp column and remove timezone
		...
		print(df_merged_set['metrics_hour'], df_merged_set['start_date'])
		...
		columns_to_keep = ['impressions', 'created_by_x', 'merchant_id', 'slot_names', 
			'container_type', 'component_name', 'component_namespace', 'guarantee_percentage', 
			'site', 'weblab', 'bullseye', 'container_id', 'days_interval', 'hours_interval', 'seconds_interval']

		# We create the preprocessing pipelines for both numeric and categorical data
		categoricalCols = ['created_by_x', 'merchant_id', 'slot_names', 'container_type',
							 'component_name', 'component_namespace', 'site', 'weblab', 'bullseye', 'container_id']
		...
		print(df_merged_set.columns)
		X, Y = df_merged_set.iloc[:,1:], df_merged_set.iloc[:,0]

		# Model is still linear but it has the poly features now 
		polynomial_features= PolynomialFeatures(degree=DEGREE)
		#X_ploy = polynomial_features.fit_transform(X)

		clf.fit(X, Y)
	saveModel(clf, 'polynomical', DEGREE)
